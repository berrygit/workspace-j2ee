...
public class HeapWithGeneric<T extends Comparable<T>> {

	private T[] numbers = null;

	int size = 0;

	public HeapWithGeneric(T[] numbers, int size) {
	...
	public static void main(String[] args) {
		...
		Integer[] numbers = {17, 63, 43, 64, 58, 54, 97, 65, 7, 2, 65, 67, 95, 34, 5, 976, 465, 35, 34, 64, 34, 643,
				15, 23, 1, 46, 32, 35, 345, 23, 9, 4, 62, 234, 77};

		new HeapWithGeneric<Integer>(numbers, numbers.length - 1).sort();
	...
	private void shiftDown(int position) {
		...
		while (hasLeftChild(position)) {
			...
			if (hasRightChild(position)) {
				if (!satisfyHeap(rightChild, position) && numbers[rightChild].compareTo(numbers[leftChild]) > 0) {
	...
	private boolean satisfyHeap(int position, int parentPosition) {
		if (numbers[position].compareTo(numbers[parentPosition]) < 1) {
			return true;
		}
		return false;
	}
